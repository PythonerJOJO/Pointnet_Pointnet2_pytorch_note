...
def parse_args():
    ...
    parser.add_argument('--num_category', default=40, type=int, choices=[10, 40],  help='在ModelNet10/40数据集上训练')
    ...
    parser.add_argument('--num_point', type=int, default=1024, help='每批点云包含的点数')
    parser.add_argument('--optimizer', type=str, default='Adam', help='优化器类型（Adam/SGD）')
    parser.add_argument('--log_dir', type=str, default=None, help='实验日志根目录（若未指定则自动生成时间戳目录）')
    ...
    parser.add_argument('--process_data', action='store_true', default=False, help='是否离线预处理数据（加速加载）')
    parser.add_argument('--use_uniform_sample', action='store_true', default=False, help='是否使用均匀采样策略')
...
def test(model, loader, num_class=40):
    ...
    class_acc = np.zeros((num_class, 3))  # 类别准确率统计矩阵 [类别索引, [正确比例累加, 样本数累加, 最终准确率]]
    classifier = model.eval()  # 设置模型为评估模式（关闭dropout/batchnorm等）

    for j, (points, target) in tqdm(enumerate(loader), total=len(loader), desc="测试进度"):
        # 数据设备转移
        if not args.use_cpu:
        ...
        pred_choice = pred.data.max(1)[1]  # 获取每个样本的预测类别（取logits最大值的索引）

        # 按类别统计准确率
        for cat in np.unique(target.cpu()):  # 遍历当前批次中的所有类别
            # 计算当前类别正确预测的比例并累加到class_acc
            correct_mask = pred_choice[target == cat].eq(target[target == cat].long().data)
            classacc = correct_mask.cpu().sum().item() / float(points[target == cat].size()[0])
    ...
    # 计算最终类别平均准确率
    class_acc[:, 2] = class_acc[:, 0] / class_acc[:, 1]  # 每个类别准确率 = 正确比例总和 / 样本数总和
...
def main(args):
    ...
    '''超参数配置'''
    os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu  # 设置可见的GPU设备（支持多卡，如"0,1"）
    ...
    model_module = importlib.import_module(args.model)  # 动态导入模型模块（如pointnet_cls）
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.7)  # 学习率衰减策略
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), desc="训练进度"):
            optimizer.zero_grad()  # 梯度清零

            # 数据增强（转换为numpy数组以使用provider模块函数）
            points = points.data.numpy()
            points = provider.random_point_dropout(points)  # 随机丢弃部分点
            points[:, :, 0:3] = provider.random_scale_point_cloud(points[:, :, 0:3])  # 随机缩放坐标
            points[:, :, 0:3] = provider.shift_point_cloud(points[:, :, 0:3])  # 随机平移坐标
            ...
            # 前向传播与损失计算
            pred, trans_feat = classifier(points)  # 获取预测结果和特征变换矩阵（用于正则化）
            loss = criterion(pred, target.long(), trans_feat)  # 计算损失（含特征对齐正则项）
        ...
        with torch.no_grad():
            ...
            log_string(f'测试实例准确率: {instance_acc:.6f}, 类别准确率: {class_acc:.6f}')
            log_string(f'最佳实例准确率: {best_instance_acc:.6f}, 类别准确率: {best_class_acc:.6f}')
